...
export default function StudentSocial() {
    ...
    const pickMedia = async (type: 'image' | 'video') => {
        ...
        if (!permission.granted) return Alert.alert('Permission Denied', 'Allow gallery access to upload.');
    ...
    const handleAddPost = async () => {
        ...
        } catch (err) { Alert.alert('Error', 'Failed to share post.'); } finally { setIsPosting(false); }
    };
    ...
    const handleReportPost = async () => {
        ...
        try {
            const resp = await socialApi.reportPost({
    ...
    const handleLikePost = async (postId: string) => {
        ...
        try {
            const response = await socialApi.likePost({ postId, userId: currentUser.uid, userName: currentUser.fullName });
            if (response.data.success) loadData();
        } catch (err) { }
    };

    const handleInlineComment = async (postId: string) => {
        const commentText = inlineCommentState[postId];
        if (!commentText?.trim() || !currentUser) return;
        try {
            const response = await socialApi.commentPost({ postId, userId: currentUser.uid, userName: currentUser.fullName, text: commentText.trim() });
            if (response.data.success) { setInlineCommentState({ ...inlineCommentState, [postId]: '' }); loadData(); }
        } catch (err) { Alert.alert('Error', 'Failed to add comment.'); }
    };
    ...
    const renderedPosts = filterByMe && currentUser ? posts.filter(p => p.userId === currentUser.uid) : posts;
        ...
        return (
            <View style={[styles.fullScreenPost, { backgroundColor: colors.card }]}>
                <View style={styles.postHeader}>
                    ...
                    <View style={{ flex: 1 }}><Text style={[styles.userName, { color: colors.text }]}>{item.userName}</Text><Text style={[styles.postTime, { color: colors.textSecondary }]}>{formatTime(item.createdAt)}</Text></View>
                ...
                <View style={[styles.inlineCommentRow, { borderTopColor: colors.border }]}>
                    <View style={[styles.tinyAvatar, { backgroundColor: isDark ? '#1A2744' : '#F0F9FF' }]}><Text style={[styles.tinyAvatarText, { color: Colors.primary }]}>{currentUser?.fullName?.charAt(0)}</Text></View>
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <AppHeader title="Learnova" toggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} showPost={true} onPostPress={() => setIsModalVisible(true)} onAvatarPress={() => setFilterByMe(!filterByMe)} notificationCount={notifications.filter(n => !n.read).length} onNotificationsPress={() => setIsNotifModalVisible(true)} />
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 20}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<View style={styles.centerContent}><Ionicons name="people-outline" size={60} color={colors.border} /><Text style={{ color: colors.textSecondary }}>No posts yet.</Text></View>}
            ...
            <Modal visible={isModalVisible} animationType="fade" transparent={true}>
                <Pressable style={styles.modalOverlay} onPress={() => setIsModalVisible(false)}>
                    <View style={[styles.modalContent, { backgroundColor: colors.card, alignSelf: 'center', width: '90%', minHeight: 400, borderRadius: 25 }]} onStartShouldSetResponder={() => true}>
                        ...
                        <TextInput style={[styles.postInput, { color: colors.text, flex: 1 }]} placeholder="What's happening?" placeholderTextColor={colors.textSecondary} multiline value={newPostText} onChangeText={setNewPostText} />
            ...
            <Modal visible={isOptionsMenuVisible} animationType="fade" transparent={true}>
                <Pressable style={[styles.modalOverlay, { width: '100%' }]} onPress={() => setIsOptionsMenuVisible(false)}><View style={[styles.menuContent, { backgroundColor: colors.card }]} onStartShouldSetResponder={() => true}><TouchableOpacity style={styles.menuItem} onPress={() => { setIsOptionsMenuVisible(false); setIsReportModalVisible(true); }}><Ionicons name="flag-outline" size={24} color={colors.text} /><Text style={[styles.menuText, { color: colors.text }]}>Report Post</Text></TouchableOpacity>{selectedPost && currentUser && selectedPost.userId === currentUser.uid && (<TouchableOpacity style={styles.menuItem} onPress={() => handleDeletePost(selectedPost.id)}><Ionicons name="trash-outline" size={24} color="#FF4444" /><Text style={[styles.menuText, { color: '#FF4444' }]}>Delete Post</Text></TouchableOpacity>)}</View></Pressable>
            </Modal>

            <Modal visible={isReportModalVisible} animationType="slide" transparent={true}><Pressable style={styles.modalOverlay} onPress={() => setIsReportModalVisible(false)}><View style={[styles.reportContent, { backgroundColor: colors.card }]} onStartShouldSetResponder={() => true}><Text style={[styles.modalTitle, { color: colors.text, marginBottom: 5 }]}>Report Post</Text><Text style={[styles.label, { color: colors.textSecondary }]}>Why are you reporting this post?</Text><View style={styles.reportTypes}>{['Spam', 'Harassment', 'False Info', 'Inappropriate'].map((type) => (<TouchableOpacity key={type} style={[styles.typeBtn, reportType === type && { backgroundColor: Colors.primary, borderColor: Colors.primary }]} onPress={() => setReportType(type)}><Text style={[styles.typeText, { color: reportType === type ? '#FFF' : colors.text }]}>{type}</Text></TouchableOpacity>))}</View><TextInput style={[styles.reportInput, { backgroundColor: isDark ? '#333' : '#F5F5F5', color: colors.text }]} placeholder="Provide details (optional)..." placeholderTextColor={colors.textSecondary} multiline value={reportDesc} onChangeText={setReportDesc} /><View style={styles.modalFooter}><TouchableOpacity style={styles.cancelBtn} onPress={() => setIsReportModalVisible(false)}><Text style={{ color: colors.textSecondary }}>Cancel</Text></TouchableOpacity><TouchableOpacity style={[styles.submitBtn, { backgroundColor: Colors.primary }]} onPress={handleReportPost}><Text style={{ color: '#FFF', fontWeight: 'bold' }}>Submit Report</Text></TouchableOpacity></View></View></Pressable></Modal>
            <Modal visible={isCommentModalVisible} animationType="slide" transparent={true}><Pressable style={styles.modalOverlay} onPress={() => setIsCommentModalVisible(false)}><View style={[styles.commentModalFull, { backgroundColor: colors.card }]} onStartShouldSetResponder={() => true}><View style={styles.modalHeader}><TouchableOpacity onPress={() => setIsCommentModalVisible(false)}><Ionicons name="close" size={24} color={colors.text} /></TouchableOpacity><Text style={[styles.modalTitle, { color: colors.text }]}>All Comments</Text><View style={{ width: 24 }} /></View><ScrollView>{selectedPost?.comments ? Object.keys(selectedPost.comments).map(k => { const c = selectedPost.comments![k]; return (<View key={k} style={styles.commentItem}><View style={[styles.tinyAvatar, { backgroundColor: Colors.primary }]}><Text style={[styles.tinyAvatarText, { color: '#FFF' }]}>{c.userName.charAt(0)}</Text></View><View style={[styles.commentBubble, { backgroundColor: isDark ? '#333' : '#F0F2F5' }]}><Text style={[styles.commentName, { color: colors.text }]}>{c.userName}</Text><Text style={{ color: colors.text }}>{c.text}</Text></View></View>); }) : <View style={{ alignItems: 'center', padding: 20 }}><Text style={{ color: colors.textSecondary }}>No comments yet.</Text></View>}</ScrollView></View></Pressable></Modal>
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <View style={{ flex: 1, marginLeft: 10 }}>
                                            ...
                                            <Text style={{ color: colors.textSecondary, fontSize: 11, marginTop: 4 }}>{formatTime(notif.createdAt)}</Text>
                                        </View>
...
const styles = StyleSheet.create({
    ...
    postContent: { paddingHorizontal: 0 },
    fullWidthPostImage: { width: '100%', height: width * 0.8, backgroundColor: '#000' },
    ...
    modalContent: { alignSelf: 'center', width: '100%', padding: 20 },
    modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 15 },
    ...
    reportInput: { height: 100, borderRadius: 15, padding: 15, textAlignVertical: 'top', fontSize: 14 },
    ...
    mediaOptionsRow: { flexDirection: 'row', justifyContent: 'center', paddingVertical: 15, gap: 40 },
